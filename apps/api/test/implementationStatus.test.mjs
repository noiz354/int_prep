...
test('U0 claims no user-usable or production-deployed capabilities', () => {
  ...
  assert.equal(interview.counts.mocked, 8);
  assert.equal(interview.counts.local_only, 92);
  assert.equal(all.counts.mocked, 27);
  assert.equal(all.counts.local_only, 135);
